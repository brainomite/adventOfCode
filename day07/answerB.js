...
const total = input
  .map((line) => {
    ...
    const hand = mapCards(cards);
    return { hand, bid, cards };
  })
  .sort((handA, handB) => {
    const gradeA = gradeHand(handA);
    const gradeB = gradeHand(handB);
...
function mapCards(cards) {
  return cards.split("").reduce((accum, card) => {
    return {
      ...accum,
...
function gradeHand(game) {
  if (!game.cards.includes("J")) return gradePossibleHand(game.hand);
  return ["1", "2", "3", "4", "5", "6", "7", "8", "9", "T", "Q", "K", "A"]
    .map((card) => {
      const cards = game.cards.replaceAll("J", card);
      return gradePossibleHand(mapCards(cards));
    })
    .sort((a, b) => b - a)[0];

  // .reduce((accum, card) => {
  //   const grade = gradePossibleHand(game.cards.replaceAll("J", card));
  //   if (grade > accum) return grade;
  //   return accum;
  // }, 0);
}

function gradePossibleHand(hand) {
